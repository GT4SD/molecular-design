...
def main(
    ...
    smiles_language.set_smiles_transforms(augment=False, sanitize=False)
    ...
    logger.info(
        f"Test language: {smiles_language.transform_smiles} and {smiles_language.transform_encoding}"
    )
    ...
    logger.info(f"Batch size = {params['batch_size']}, dataset size = {len(dataset)}")
    if confidence:
