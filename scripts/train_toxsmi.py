...
def main(
    ...
    smiles_dataset = SMILESTokenizerDataset(
        ...
        iterate_dataset=params.get("iterate_dataset", True),
    )
    ...
    if params.get("keep_probs"):
        ...
        keep_probs = params.get("keep_probs", [1] * len(freqs.unique()))
        mapper = {"low": 0, "medium": 1, "high": 2}
        weights = [keep_probs[mapper[x]] for x in freqs]
    ...
    if finetune_path:
        if os.path.isfile(finetune_path):
            # Check whether pretrained and finetuned model have same number of tasks
            with open(
                os.path.join(
                    os.path.dirname(os.path.dirname(finetune_path)), "model_params.json"
                ),
                "r",
            ) as f:
                backbone_params = json.load(f)
            if backbone_params["num_tasks"] != params["num_tasks"]:
                logger.info("Adjusting number of tasks in model to load it safely")
                model.num_tasks = backbone_params["num_tasks"]
                model.final_dense = EnsembleLayer(
                    typ=params.get("ensemble", "score"),
                    input_size=model.hidden_sizes[-1],
        ...
        # Reverting the last layer change
        if backbone_params["num_tasks"] != params["num_tasks"]:
            logger.info("Revering number of tasks in model")
            act_fn = (
                ACTIVATION_FN_FACTORY["sigmoid"]
                if "cross" in model.loss_name
    ...
    performer = PerformanceLogger(
        ...
        beta=0.5,  # higher weight for precision than for recall
    )
